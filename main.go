...
type tree struct {
	value int
	age   int // P spalenia drzewa jest odwrotnie proporcjalne do jego wieku, gdy drzewo ma 100 lat to P = 1%, gdy 1 rok to P = 100%
}
...
func main() {
	var forest forest
	forest.populateForest(0.59, 30, 300)
	forest.lightningStrike(true)
	fmt.Printf("%.2f%%\n", forest.calculateBurnRatio()*100)
}
